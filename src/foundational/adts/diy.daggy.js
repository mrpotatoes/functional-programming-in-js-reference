...
diyDaggy.tagged = (typeName, fields) => {
...
diyDaggy.taggedSum = (typeName, constructors) => {
  let typeRep = function () { }

  Object.keys(constructors).forEach(name => {
    typeRep[name] = diyDaggy.tagged(typeName + '.' + name, constructors[name])
